...
%
%-----SIMULATION IN MATLAB USING QUANCER QBot2 ENVIROMENT--------------------------
%
% #################################################################################
% Edited by Alexis Marino (mrnlsf96p01z605k@studenti.unical.it) September, 8th 2023
% #################################################################################
%
...
% Computing the error of the system with respect to the reference
Err0= [X_k;Y_k;v(1)*cos(theta_k);v(1)*sin(theta_k)]-[xr(0);yr(0);0;0];

% control computing for the worse case scenario
...
for i=0:Ts:Tsim
    ...
    %[Q_str,Y_str] = RHC_OA(Err,A,B,H_theta_v,Q0,Qe,Qu); % yalmip solver
   
    [Q_str,Y_str] = RHC_Dy_lin_mincx(Err,A,B,H_theta_v,Qe,Qu,Q0); % mincx solver
    ...
    % Apply the control acctions to the real plant in continuos time ---------------------------
